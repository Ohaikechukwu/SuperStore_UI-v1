          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_430px]">
            ...
            <section className="flex min-h-[620px] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="border-t border-slate-100 bg-slate-950 p-5 text-white">
                ...
                <div className="mt-2 flex items-center justify-between gap-3 text-sm text-slate-300"><span>VAT / tax</span><label className="flex items-center gap-2"><select aria-label="VAT or tax rate" value={taxRate} onChange={(event) => setTaxRate(event.target.value)} className="rounded-lg border border-slate-600 bg-white/10 px-2 py-1 text-xs font-bold text-white"><option value="0">None</option>{approvedTaxRates.map((rate) => <option key={rate.name} value={String(rate.rate)}>{rate.name} {rate.rate}%</option>)}</select></label><span>{formatMoney(taxAmount)}</span></div>
