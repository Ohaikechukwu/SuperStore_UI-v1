...
export function TenantLoginPage({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    catch { if (platformAdminLogin) setNotFound(true); else setError("We could not sign you in. Check your email and password."); }
  ...
  if (notFound) return <main className="grid min-h-screen place-items-center bg-slate-50 p-6"><section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm"><p className="text-6xl font-bold tracking-tight text-slate-900">404</p><h1 className="mt-4 text-xl font-bold text-slate-900">Page not found</h1><p className="mt-2 text-sm leading-6 text-slate-500">The page you requested could not be found.</p><Link href="/" className="mt-6 inline-flex rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white">Return home</Link></section></main>;
