        ...
        <div className="mx-auto max-w-[1280px] space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row">
              <FormSelect aria-label="Filter inventory by branch"
                value={branchId}
                onChange={(event) => {
...
type TransferSummary = { id: string; status: string; reference: string | null; source_branch_id: string; destination_branch_id: string; created_at: string; lines: Array<{ product_id: string; stock_code: string; name: string; quantity: string; batch_number: string | null }> };
...
function InventoryTransferPanel() {
  ...
  const branchName = (branchId: string) => branches.find((branch) => branch.id === branchId)?.name || "—";
  const loadHistory = () => { void api.get<TransferSummary[]>("/api/v1/inventory/transfers?limit=20").then(setHistory).catch(() => setHistory([])); };
  ...
  return <section className="rounded-3xl border border-sky-100 bg-white p-6 shadow-sm"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-sky-700">Inter-branch stock</p><h2 className="mt-1 text-xl font-bold">Transfer inventory</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">Move stock between branches. Choose an exact batch when required; otherwise stock is allocated by earliest usable expiry first.</p></div><ArrowRightLeft className="text-sky-600" /></div>{error && <p className="mt-4 rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700">{error}</p>}{notice && <p className="mt-4 rounded-xl bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{notice}</p>}<form onSubmit={submit} className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><AppSelect value={sourceBranchId} onChange={setSourceBranchId} options={[{ value: "", label: "From branch" }, ...branches.map((branch) => ({ value: branch.id, label: `From: ${branch.name}` }))]} placeholder="From branch" /><AppSelect value={destinationBranchId} onChange={setDestinationBranchId} options={[{ value: "", label: "To branch" }, ...branches.filter((branch) => branch.id !== sourceBranchId).map((branch) => ({ value: branch.id, label: `To: ${branch.name}` }))]} placeholder="To branch" /><AppSelect value={productId} onChange={setProductId} options={[{ value: "", label: "Select product" }, ...balances.filter((item) => Number(item.quantity) > 0).map((item) => ({ value: item.product_id, label: `${item.name} · ${item.quantity} available` }))]} placeholder="Select product" /><input required value={quantity} onChange={(event) => setQuantity(event.target.value)} type="number" min="0.001" step="0.001" placeholder="Quantity" className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm" /><input value={batchNumber} onChange={(event) => setBatchNumber(event.target.value)} placeholder="Batch number (optional — otherwise FEFO)" className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm md:col-span-2 xl:col-span-3" /><input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="Transfer note / dispatch reference" className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm" /><button disabled={busy || !sourceBranchId || !destinationBranchId || !productId} className="min-h-11 rounded-xl bg-sky-600 px-4 text-sm font-bold text-white hover:bg-sky-700 disabled:opacity-40">{busy ? "Posting transfer…" : "Post transfer"}</button></form>
    <div className="mt-8 border-t border-slate-100 pt-5"><h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Recent transfers</h3>{history.length > 0 ? <div className="mt-3 overflow-x-auto"><table className="w-full min-w-[640px] text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th className="p-3">Posted</th><th className="p-3">Movement</th><th className="p-3">Items</th><th className="p-3">Reference</th><th className="p-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100">{history.map((transfer) => <tr key={transfer.id}><td className="p-3 text-slate-600">{new Date(transfer.created_at).toLocaleString()}</td><td className="p-3 font-semibold">{branchName(transfer.source_branch_id)} <span className="text-slate-400">→</span> {branchName(transfer.destination_branch_id)}</td><td className="p-3 text-slate-600">{transfer.lines.map((line) => `${line.name} × ${line.quantity}`).join(", ") || "—"}</td><td className="p-3 text-slate-600">{transfer.reference || "—"}</td><td className="p-3"><span className="rounded-full bg-sky-50 px-2.5 py-1 text-xs font-bold capitalize text-sky-700">{transfer.status}</span></td></tr>)}</tbody></table></div> : <p className="mt-3 text-sm text-slate-500">No transfers posted yet.</p>}</div>
  </section>;
}

type CountSummary = { id: string; branch_id: string; branch_name: string; status: string; notes: string | null; line_count: number; created_at: string };
type CountLine = { product_id: string; stock_code: string; name: string; expected_quantity: string; counted_quantity: string; variance: string };
type CountDetail = { id: string; branch_id: string; status: string; notes: string | null; created_at: string; journal_entry_id: string | null; lines: CountLine[] };

function InventoryCountPanel() {
  const [branches, setBranches] = useState<Branch[]>([]);
  const [branchId, setBranchId] = useState("");
  const [balances, setBalances] = useState<Balance[]>([]);
  const [rows, setRows] = useState<Array<{ product_id: string; counted: string }>>([]);
  ...
  const bookFor = (productId: string) => balances.find((item) => item.product_id === productId);
  ...
  useEffect(() => { void api.get<CountSummary[]>("/api/v1/inventory/counts?limit=20").then(setHistory).catch(() => setHistory([])); }, []);
  useEffect(() => {
    if (!branchId) return;
    void api.get<Balance[]>(`/api/v1/inventory/balances/${branchId}`).then((items) => { setBalances(items); setRows([]); setPicker(""); }).catch(() => setBalances([]));
  }, [branchId]);
  function addRow() {
    if (!picker || rows.some((row) => row.product_id === picker)) return;
    setRows((current) => [...current, { product_id: picker, counted: "" }]);
    setPicker("");
  }
  async function openDetail(countId: string) {
    if (detail?.id === countId) { setDetail(null); return; }
    try { setDetail(await api.get<CountDetail>(`/api/v1/inventory/counts/${countId}`)); }
    catch (caught) { setError(caught instanceof ApiError ? caught.message : "Unable to load the stock count."); }
  }
  async function submit(event: FormEvent) {
    event.preventDefault();
    const lines = rows.filter((row) => row.counted !== "").map((row) => ({ product_id: row.product_id, counted_quantity: row.counted }));
    if (!lines.length) { setError("Enter a counted quantity for at least one product before posting."); return; }
    setBusy(true); setError("");
    try {
      await api.post("/api/v1/inventory/counts", { branch_id: branchId, notes: notes.trim() || null, lines });
      setNotice("Stock count posted. Shortages were removed from their real FEFO batches; surpluses are marked unbatched for follow-up.");
      setRows([]); setNotes(""); setDetail(null);
      setBalances(await api.get<Balance[]>(`/api/v1/inventory/balances/${branchId}`));
      api.get<CountSummary[]>("/api/v1/inventory/counts?limit=20").then(setHistory).catch(() => undefined);
    } catch (caught) { setError(caught instanceof ApiError ? caught.message : "Unable to post stock count."); }
    finally { setBusy(false); }
  }
  const varianceTotal = rows.reduce((sum, row) => row.counted === "" ? sum : sum + Number(row.counted) - Number(bookFor(row.product_id)?.quantity || 0), 0);
  return <section className="rounded-3xl border border-violet-100 bg-white p-6 shadow-sm"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-violet-700">Physical verification</p><h2 className="mt-1 text-xl font-bold">Count sheet</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">Count a whole sheet of products in one posting. Book quantities are shown beside each entry, the total variance is computed live, and posting creates the inventory variance movement.</p></div><ClipboardCheck className="text-violet-600" /></div>{error && <p className="mt-4 rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700">{error}</p>}{notice && <p className="mt-4 rounded-xl bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{notice}</p>}<form onSubmit={submit} className="mt-5 grid gap-4 md:grid-cols-2"><AppSelect value={branchId} onChange={setBranchId} options={[{ value: "", label: "Select branch" }, ...branches.map((branch) => ({ value: branch.id, label: branch.name }))]} placeholder="Select branch" /><div className="flex gap-2"><AppSelect aria-label="Add product to the count sheet" value={picker} onChange={setPicker} options={[{ value: "", label: "Add a product to count" }, ...balances.filter((item) => !rows.some((row) => row.product_id === item.product_id)).map((item) => ({ value: item.product_id, label: `${item.name} · book ${item.quantity}` }))]} placeholder="Add a product" /><button type="button" onClick={addRow} disabled={!picker} className="min-h-11 shrink-0 rounded-xl border border-violet-200 px-4 text-sm font-bold text-violet-700 disabled:opacity-40">Add</button></div></form>{rows.length > 0 && <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th className="p-3">Product</th><th className="p-3">Book</th><th className="p-3">Counted</th><th className="p-3">Variance</th><th className="p-3"></th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((row) => { const book = bookFor(row.product_id); const variance = row.counted === "" ? null : Number(row.counted) - Number(book?.quantity || 0); return <tr key={row.product_id}><td className="p-3 font-semibold">{book?.name || row.product_id}</td><td className="p-3 text-slate-600">{book?.quantity ?? "—"}</td><td className="p-3"><input value={row.counted} onChange={(event) => setRows((current) => current.map((item) => item.product_id === row.product_id ? { ...item, counted: event.target.value } : item))} type="number" min="0" step="0.001" aria-label={`Counted quantity for ${book?.name || row.product_id}`} className="w-32 rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-violet-600" /></td><td className={"p-3 font-bold " + (variance == null || variance === 0 ? "text-slate-400" : variance < 0 ? "text-rose-700" : "text-emerald-700")}>{variance == null ? "—" : variance > 0 ? `+${variance}` : variance}</td><td className="p-3 text-right"><button type="button" onClick={() => setRows((current) => current.filter((item) => item.product_id !== row.product_id))} className="text-xs font-bold text-slate-500 hover:text-rose-700">Remove</button></td></tr>; })}</tbody></table></div>}{rows.length > 0 && <p className="mt-2 text-xs text-slate-500">Total variance: <strong className={varianceTotal < 0 ? "text-rose-700" : varianceTotal > 0 ? "text-emerald-700" : "text-slate-500"}>{varianceTotal > 0 ? `+${varianceTotal}` : varianceTotal}</strong> units across the sheet.</p>}<form onSubmit={submit} className="mt-4 grid gap-4 md:grid-cols-[1fr_auto]"><input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Count sheet or investigation note (recommended)" className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm" /><button disabled={busy || !branchId} className="min-h-11 rounded-xl bg-violet-600 px-6 text-sm font-bold text-white hover:bg-violet-700 disabled:opacity-40">{busy ? "Posting count…" : "Post count sheet"}</button></form>
    <div className="mt-8 border-t border-slate-100 pt-5"><h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Recent counts</h3>{history.length > 0 ? <div className="mt-3 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th className="p-3">Posted</th><th className="p-3">Branch</th><th className="p-3">Lines</th><th className="p-3">Note</th><th className="p-3"></th></tr></thead><tbody className="divide-y divide-slate-100">{history.map((count) => <tr key={count.id}><td className="p-3 text-slate-600">{new Date(count.created_at).toLocaleString()}</td><td className="p-3 font-semibold">{count.branch_name}</td><td className="p-3">{count.line_count}</td><td className="p-3 text-slate-600">{count.notes || "—"}</td><td className="p-3 text-right"><button onClick={() => void openDetail(count.id)} className="text-xs font-bold text-violet-700">{detail?.id === count.id ? "Hide" : "Review"}</button></td></tr>)}</tbody></table></div> : <p className="mt-3 text-sm text-slate-500">No stock counts posted yet.</p>}
    {detail && <div className="mt-4 rounded-2xl border border-violet-100 bg-violet-50/40 p-4"><p className="text-xs font-bold uppercase tracking-wider text-violet-700">Count of {new Date(detail.created_at).toLocaleString()} · {detail.status}</p>{detail.notes && <p className="mt-1 text-sm text-slate-600">Note: {detail.notes}</p>}<div className="mt-3 overflow-x-auto"><table className="w-full min-w-[520px] text-left text-sm"><thead className="border-b bg-white/70 text-xs uppercase tracking-wider text-slate-500"><tr><th className="p-2">Product</th><th className="p-2">Book</th><th className="p-2">Counted</th><th className="p-2">Variance</th></tr></thead><tbody className="divide-y divide-slate-100">{detail.lines.map((line) => <tr key={line.product_id}><td className="p-2 font-semibold">{line.name} <span className="font-mono text-xs text-slate-400">{line.stock_code}</span></td><td className="p-2">{line.expected_quantity}</td><td className="p-2">{line.counted_quantity}</td><td className={"p-2 font-bold " + (Number(line.variance) < 0 ? "text-rose-700" : Number(line.variance) > 0 ? "text-emerald-700" : "text-slate-500")}>{Number(line.variance) > 0 ? `+${line.variance}` : line.variance}</td></tr>)}</tbody></table></div>{detail.journal_entry_id && <p className="mt-2 text-xs text-slate-500">Variance journal posted to the ledger (entry {detail.journal_entry_id.slice(0, 8)}…).</p>}</div>}</div>
  </section>;
}
