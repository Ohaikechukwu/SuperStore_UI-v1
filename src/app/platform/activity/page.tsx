...
type AuditEvent = AuditPresentation & { id: string; tenant_id: string; action: string; entity_type: string; entity_id: string; actor_id: string | null; reason: string | null; created_at: string };
...
export default function PlatformActivityPage() {
  ...
  return <DashboardShell title="Platform activity" subtitle="Auditable cross-tenant governance, licensing, and operational changes."><PermissionGate permission="tenant.read"><div className="mx-auto max-w-[1200px] space-y-6"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-teal-600">Governance</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">Platform activity</h1></div>{error && <p className="rounded-2xl bg-rose-50 px-4 py-3 text-sm text-rose-700">{error}</p>}{allowed && <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-xs font-bold uppercase tracking-wider text-teal-600">Audit trail</p><div className="mt-4 divide-y divide-slate-100">{events.map((event) => <article key={event.id} className="flex flex-col gap-2 py-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-bold text-slate-800">{auditSummary(event)}</p><p className="mt-1 text-xs text-slate-500">{event.entity_type} · {tenants.find((tenant) => tenant.id === event.tenant_id)?.name || event.tenant_id} · {event.entity_id}</p>{event.reason && <p className="mt-2 text-xs text-slate-500">Reason: {event.reason}</p>}</div><p className="text-xs text-slate-400">{auditWhen(event)} · {event.actor?.name || "Actor not recorded"}</p></article>)}{!events.length && <p className="py-8 text-center text-sm text-slate-500">No platform activity recorded.</p>}</div></section>}</div></PermissionGate></DashboardShell>;
}
