...
export default function ProductsPage() {
  ...
  async function autoClassifyLegacy() { if (!window.confirm("Classify every product that has never been assigned an operational area as Pharmacy? Existing Store / Pharmacy / Laboratory assignments are never overwritten.")) return; try { const result = await api.post<{ updated: number }>("/api/v1/catalog/products/service-area/auto-classify", {}); setNotice(result.updated === 0 ? "No unclassified products were found." : `${result.updated} legacy product${result.updated === 1 ? "" : "s"} classified as pharmacy.`); await load(page.offset, true); } catch (caught) { setError(errorMessage(caught, "Unable to auto-classify products.")); } }
  ...
  return <DashboardShell title="Product catalogue" subtitle="A scalable procurement catalogue; POS availability begins with a received PO."><PermissionGate permission="catalog.product.read"><div className="mx-auto max-w-[1280px] space-y-6">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-600">Service areas</p><h2 className="mt-1 text-xl font-bold">Route products to the right operational workspace</h2><p className="mt-1 text-sm text-slate-500">Choose an area on a product row, or select several rows to update them together. Product category never changes a workspace automatically.</p></div><div className="flex flex-wrap gap-2 text-xs font-bold"><span className="rounded-full bg-slate-100 px-3 py-2">Store {serviceSummary?.counts.store || 0}</span><span className="rounded-full bg-teal-50 px-3 py-2 text-teal-700">Pharmacy {serviceSummary?.counts.pharmacy || 0}</span><span className="rounded-full bg-violet-50 px-3 py-2 text-violet-700">Laboratory {serviceSummary?.counts.laboratory || 0}</span>{can(authorization, "catalog.product.update") && <button onClick={() => void autoClassifyLegacy()} className="rounded-xl border border-slate-200 bg-white px-3 py-2 font-bold text-slate-600 hover:border-teal-300 hover:text-teal-700">Auto-classify legacy items</button>}</div></div>{selectedIds.length > 0 && <div className="mt-4 flex flex-wrap items-center gap-2 rounded-2xl bg-slate-50 p-3 text-sm"><strong>{selectedIds.length} selected</strong><button onClick={() => void classify("store")} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold">Set store</button><button onClick={() => void classify("pharmacy")} className="rounded-xl border border-teal-200 px-3 py-2 text-xs font-bold text-teal-700">Set pharmacy</button><button onClick={() => void classify("laboratory")} className="rounded-xl border border-violet-200 px-3 py-2 text-xs font-bold text-violet-700">Set laboratory</button></div>}</section>
