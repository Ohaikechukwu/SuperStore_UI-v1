...
export default function Page() {
  ...
  async function load() { setBusy(true); setError(""); try { setItems(await api.get<Referral[]>("/api/v1/hospital/clinical/referrals?purpose=care_coordination")); } catch (caught) { setError(caught instanceof ApiError ? caught.message : "Unable to load referrals."); } finally { setBusy(false); } }
  useEffect(() => {
    let active = true;
    void api.get<Referral[]>("/api/v1/hospital/clinical/referrals?purpose=care_coordination")
      .then((items) => { if (active) setItems(items); })
      .catch((caught) => { if (active) setError(caught instanceof ApiError ? caught.message : "Unable to load referrals."); })
