...
export default function Page() {
    ...
    async function voidInvoice(invoice: Invoice) { const reason = window.prompt("Reason for voiding this invoice? Only unpaid invoices can be voided."); if (!reason || reason.trim().length < 2) return; try {
        await api.post(`/api/v1/hospital/billing/invoices/${invoice.id}/void`, { reason });
        setNotice("Invoice voided; its ledger journal was reversed.");
        await load();
    }
    catch (caught) {
        setError(caught instanceof ApiError ? caught.message : "Unable to void the invoice.");
    } }
