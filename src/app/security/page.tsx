...
import { auditSummary, auditWhen, type AuditPresentation } from "@/lib/audit-trail";
...
type AuditEvent = AuditPresentation & {
...
type ClinicalAccessEvent = { id: string; patient_id: string; actor_id: string; action: string; purpose: string | null; fields_accessed: string | null; created_at: string };

const moneyKeys = new Set(["opening_cash", "closing_cash", "expected_cash", "variance", "gross_sales", "refund_total", "net_sales", "total", "subtotal", "amount", "unit_cost", "unit_price", "selling_price", "cost_price"]);
...
function auditDetails(event: AuditEvent): Detail[] {
  ...
  for (const change of event.changes || []) {
    details.push({ label: labels[change.field] || humanize(change.field), value: `${valueFor(change.field, change.before)} → ${valueFor(change.field, change.after)}` });
  }
...
export default function Page() {
  ...
  return <DashboardShell title="Security & audit" subtitle="MFA, active sessions, authentication activity, and audit history"><PermissionGate permission="audit.read"><div className="mx-auto max-w-[1280px] space-y-6">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><LockKeyhole className="text-teal-600" size={21} /><div><h2 className="text-xl font-bold">Audit activity</h2><p className="mt-1 text-xs text-slate-400">Latest {events.length} actions, with staff names, affected records, and company-local times.</p></div></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-500">{events.length}</span></div><div className={`mt-5 divide-y divide-slate-100 ${auditScroll ? "max-h-[58rem] overflow-y-auto pr-2" : ""}`}>{events.map((event) => { const details = auditDetails(event); return <article key={event.id} className="py-5"><div className="flex items-start gap-4"><div className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-teal-500" /><div className="min-w-0 flex-1"><div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"><p className="font-bold text-slate-900">{auditSummary(event)}</p><p className="shrink-0 text-xs text-slate-400">{auditWhen(event)}</p></div><p className="mt-1 flex items-center gap-1 text-sm text-slate-600"><UserRound size={14} className="text-slate-400" /> {actorLabel(event)}{event.actor?.email && <span className="text-slate-400">· {event.actor.email}</span>}</p>{details.length > 0 && <dl className="mt-3 grid gap-x-6 gap-y-2 text-xs sm:grid-cols-2 lg:grid-cols-3">{details.map((detail) => <div key={detail.label} className="min-w-0"><dt className="font-semibold text-slate-400">{detail.label}</dt><dd className="mt-0.5 break-words font-medium text-slate-700">{detail.value}</dd></div>)}</dl>}{event.reason && <p className="mt-3 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">Reason: {event.reason}</p>}</div></div></article>; })}{!events.length && <p className="py-8 text-sm text-slate-500">No audit events available.</p>}</div></section>
