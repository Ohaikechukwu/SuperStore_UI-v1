...
export default function PermissionGate({ permission, module, allowedRoles, children }: { permission?: string; module?: string; allowedRoles?: string[]; children: React.ReactNode }) {
  ...
  if (permission && !can(context, permission)) return <div className="rounded-3xl border border-rose-100 bg-rose-50 p-8"><p className="text-xs font-bold uppercase tracking-[0.18em] text-rose-600">Access restricted</p><h1 className="mt-3 text-2xl font-bold text-rose-950">You do not have access to this workspace</h1><p className="mt-2 text-sm text-rose-800">Required permission: {permission}</p></div>;
  return <>{children}</>;
}
